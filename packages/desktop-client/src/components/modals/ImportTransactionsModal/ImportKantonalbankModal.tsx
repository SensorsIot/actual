...
export function ImportKantonalbankModal({
  options,
  ...
  useEffect(() => {
    async function parseFile() {
      ...
      const {
        ...
        filepath: filename,
        options: { swissBankFormat: 'kantonalbank', importNotes: true },
      });
      ...
      // Verify this is actually a Kantonalbank file
      if (metadata?.bankFormat !== 'kantonalbank') {
        setError({
          parsed: false,
          message: t('This file does not appear to be a Kantonalbank export.'),
        });
        setLoadingState(null);
        return;
      }
      ...
      // Find target account
      if (settings.kantonalbank_account) {
        const account = accounts.find(
          a => a.name === settings.kantonalbank_account && !a.closed,
        );
        if (account) {
      ...
      } else if (targetAccountId || settings.kantonalbank_account) {
  ...
  async function onImport(close: () => void) {
    ...
    // Import using Kantonalbank handler
  ...
  async function onSaveSettings() {
    await send('swiss-bank-save-import-settings', {
      settings: importSettings,
    });

    // Update target account
    const account = accounts.find(
      a => a.name === importSettings.kantonalbank_account && !a.closed,
    );
    if (account) {
      setTargetAccountId(account.id);
  ...
  return (
    <Modal
      ...
      containerProps={{ style: { width: 900 } }}
    >
      {({ state: { close } }) => (
        <>
          <ModalHeader
            title={t('Import Kantonalbank Transactions')}
            ...
            {showSettingsDialog && (
              ...
              >
                <Text style={{ fontWeight: 'bold', marginBottom: 10 }}>
                  <Trans>Configure Kantonalbank Import Settings</Trans>
                </Text>
                <Text
                ...
                <View style={{ marginBottom: 10 }}>
                  ...
                  >
                    <Text style={{ width: 180 }}>
                      <Trans>Kantonalbank Account:</Trans>
                    </Text>
                    <Select
                      value={importSettings.kantonalbank_account}
                      onChange={(e: string) =>
                        setImportSettings({
                          ...importSettings,
                ...
                <View style={{ marginTop: 15 }}>
                  <Button
                    variant="primary"
                    isDisabled={!importSettings.kantonalbank_account}
