...
export type CreatedCategoriesInfo = {
...
export type UseSwissBankImportResult = {
  ...
  setTransactionCategories: React.Dispatch<
    React.SetStateAction<TransactionCategoryMap>
  >;
  ...
  setTransactionNotes: React.Dispatch<
  ...
  ) => Promise<CreatedCategoriesInfo | null>;

  // Payee mapping save
  collectPayeeMappingsToSave: () => Array<{
    payee: string;
    category: string;
    isExpense: boolean;
  }>;
...
export function useSwissBankImport(): UseSwissBankImportResult {
  const [transactionCategories, setTransactionCategories] =
    useState<TransactionCategoryMap>(new Map());
  ...
  const onTransactionNotesChange = useCallback(
    (transactionId: string, notes: string | null) => {
  ...
  const fetchCategorySuggestions = useCallback(
    async (
      transactionsToProcess: ImportTransaction[],
    ): Promise<CreatedCategoriesInfo | null> => {
      ...
      for (const trans of transactionsToProcess) {
        const payee =
          (
            trans as ImportTransaction & {
              payee_name?: string;
              imported_payee?: string;
            }
          ).payee_name ||
          (trans as ImportTransaction & { imported_payee?: string })
            .imported_payee ||
      ...
      const payeeInputs = Array.from(payeeAmounts.entries()).map(
        ([payee, amount]) => ({
          payee,
          amount,
        }),
      );
      const matchResults = await send('swiss-bank-match-payees', {
        payees: payeeInputs,
      });
  ...
  const collectPayeeMappingsToSave = useCallback(() => {
    const payeeMappingsToSave: Array<{
      payee: string;
      category: string;
      isExpense: boolean;
    }> = [];
    ...
    if (transactionCategories.size > 0) {
      const payeeMappings = new Map<
        string,
        { category: string; isExpense: boolean }
      >();
      ...
      for (const catInfo of transactionCategories.values()) {
        ...
        const categoryChanged =
          catInfo.selectedCategory !== catInfo.proposedCategory;
        if (
          catInfo.selectedCategory &&
          catInfo.payee &&
          (isNewPayee || categoryChanged)
        ) {
      ...
      if (payeeMappings.size > 0) {
        payeeMappingsToSave.push(
          ...Array.from(payeeMappings.entries()).map(([payee, info]) => ({
            payee,
